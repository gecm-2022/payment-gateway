...
const Transaction = () => {
  const { getUserTransactions, transactions, user } = useAuth(); // Get transactions from auth context
  ...
  const renderTransactions = (type) => {
    ...
    if (type === "other") {
      ...
      filteredTransactions = transactions.filter(
        (tx) => tx.type === "recharge" || tx.type === "withdrawal"
      );
    } else {
    ...
    return filteredTransactions.map((tx) => (
      <div key={tx._id} className="border p-4 mb-2">
        ...
        <p>
          <strong>Date:</strong> {new Date(tx.createdAt).toDateString()}
        </p>
        <p>
  ...
  return (
    <div className="p-4  items-center m-auto my-1    md:w-[100vh] w-auto">
      <div className="mb-4 flex justify-center space-x-4 a">
        <button
